...
#define NUMBER_OF_ENCRYPTIONS (100000)
...
// >>>>> FUZZING total attack code profiling 1 end
  int fd = open("/home/mais_lab/openssl_098t/lib/libcrypto.so", O_RDONLY);
  ...
  sum = 0;
  char* probe[] = { 
    base + 0x170e20, base + 0x171220, base + 0x171620, base + 0x171a20
  };

  FILE *file = fopen("../collecteddata/PAPI_data.txt", "w");
  if (file == NULL) {
  ...
  int event_code1 = PAPI_NATIVE_MASK;
  int event_code2 = PAPI_NATIVE_MASK;
  int event_code3 = PAPI_NATIVE_MASK;

  PAPI_event_name_to_code("PAPI_TOT_CYC",&event_code);
  PAPI_event_name_to_code("PAPI_L1_DCM",&event_code1);
  PAPI_event_name_to_code("PAPI_L3_TCM",&event_code2);
  PAPI_event_name_to_code("PAPI_L3_TCA",&event_code3); 

  ret_val = PAPI_add_event(EventSet, PAPI_TOT_CYC);
  if (ret_val != PAPI_OK) {
    fprintf(stderr, "PAPI add event error0!\n");
    exit(1);
  }

  ret_val = PAPI_add_event(EventSet, PAPI_L1_DCM);
  if (ret_val != PAPI_OK) {
    fprintf(stderr, "PAPI add event error1!\n");
    exit(1);
  }

  ret_val = PAPI_add_event(EventSet, PAPI_L3_TCM);
  if (ret_val != PAPI_OK) {
    fprintf(stderr, "PAPI add event error2!\n");
    exit(1);
  } 
  
  ret_val = PAPI_add_event(EventSet, PAPI_L3_TCA);
  if (ret_val != PAPI_OK) {
    fprintf(stderr, "PAPI add event error2!\n");
    exit(1);
  } 
  ...
  //printf("PAPI_TOT_CYC, PAPI_L1_DCM, PAPI_L3_TCM, PAPI_L3_TCA\n");
...
// >>>>> randomize_attack fuzz end 
    ...
    if (i%100 == 0){
      // stop the PAPI
      ret_val = PAPI_read(EventSet, values);
