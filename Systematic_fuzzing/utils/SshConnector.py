...
class SshConnector:
    ...
    def execute_command_v2(self, command, password=None, wantPrint=True):
        """
        Executes a command on the remote server with optional sudo privileges and returns & prints (if wantPrint) the output.
        This method can execute both normal and sudo commands. If password is provided, the command will be executed with sudo.

        This method/function is partially written by, or written with the aid of, ChatGPT.
        """
        if self.sshConnected:
            try:
                if password:  # Prepare to use sudo
                    command = "sudo -S -p '' " + command
                    shell = self.ssh_client.invoke_shell()
                    shell.send(command + '\n')
                    shell.send(password + '\n')
                    shell.settimeout(10)  # Set a timeout for reading output to avoid hanging
                else:
                    stdin, stdout, stderr = self.ssh_client.exec_command(command)
                    stdout.channel.recv_exit_status()

                # Read the output from stdout and stderr
                output = stdout.read().decode() if not password else shell.recv(4096).decode()
                error = stderr.read().decode() if not password else ''
                ...
                logger.error("Failed to execute command: " + str(e))
                return (-1, str(e))
        else:
            bad = "Connection not established. Please connect first."
            logger.error(bad)
            return (-2, bad)  # Return -2 for not connected
