...
# pcm : LLC Misses
##############################################################################
    def increaseCacheMisses(self, increment, extraPrints=False):
        """
        Important: Call setInitialLENForCacheMisses() before calling this method!
        ...
        of increase can be controlled by how big those arrays are, aka the "LEN" (length) of the array.
        So, this function's job is to play with the "LEN" to increase cache-misses.
        
        if self.previousTriedLEN is False, the self.currentTryLEN will attempt a LEN that is > self.previousTriedLEN.
        else it will try LEN = self.currentTryLEN without caring what self.previousTriedLEN is.

        This method will scp the code to the server to be compiled.
        """
        self.__helperForCacheMisses(increment, extraPrints)

    def decreaseCacheMisses(self, increment, extraPrints=False):
        """
        Important: Call setInitialLENForCacheMisses() before calling this method!
        
        This method's job is to decrease perf's "cache-misses" counters. This is done by adding
        a function that copies data from an array to another array that is far (1MB) away in memory.
        We ran tests and found that doing so will decrease the "cache-misses" counter. The amount 
        of decrease can be controlled by how big those arrays are, aka the "LEN" (length) of the array.
        So, this function's job is to play with the "LEN" to decrease cache-misses.
        ...
        This method will scp the code to the server to be compiled.
        """
        self.__helperForCacheMisses(-1 * increment, extraPrints)

    def __helperForCacheMisses(self, increment, extraPrints=False):
        """
        Private method.
        
        Since this code is shared by so many other methods, we will use this method to reduce duplicate code.

        increment > 0 for increasing cache misses
        increment < 0 for decreasing cache misses
        """
        LEN = self.currentTryLEN

        self.currentTryLEN = self.previousTriedLEN + increment
        if self.currentTryLEN <= 0:
            logger.error(f"LEN is <= 0!")
            # return -1 # < 0 for error
            raise LenLTEZeroError("LEN is <= 0! aka the \"increment\" you picked caused LEN to go <= 0.")
        ...
        fuzz_string = f"""\
            ...
            memset(s, 0xA, {LEN}/*this number is generated by python framework*/);
            ...
            asm volatile (
                ...
                : [count] "r" ((uint64_t) {LEN}/*this number is generated by python framework*/), "S" (s), "D" (d) // Input: count, src (esi), dest (edi)
                : "%rax", "%rcx", "memory"  // Clobbered: rax, ecx, and memory to indicate memory is being modified
            );

            for (int i = 0; i < {LEN}/*this number is generated by python framework*//8/*X86_CACHELINE_SIZE_NUM_OF_B*/; i += 8*8/*X86_CACHELINE_SIZE_NUM_OF_B*/) {{
                flush((void *)(s + i));
                flush((void *)(d + i));
            }}
        ...
        self.previousTriedLEN = self.currentTryLEN
    ...
    def setInitialLENForCacheMisses(self, initialLEN):
        """
        Set the initial LEN for arrays for cache-misses' fuzz. If this method isn't called, 
        the default initial LENis 50000;
        """
        self.previousTriedLEN = initialLEN
        self.currentTryLEN = initialLEN

##############################################################################
# Module 2 
# perf: cache_references (cache miss + cache hit)
# Refers to all the cache accesses
##############################################################################
    def increaseCacheReferences(self, increment, extraPrints=False):
        """
        Important: Call setInitialLENForCacheMisses() before calling this method!
        
        This method's job is to increase perf's "cache-references" counters. This is done by adding
        a function that copies data from an array to another array that is far (1MB) away in memory.
        We ran tests and found that doing so will increase the "cache-references" counter. The amount 
        of increase can be controlled by how big those arrays are, aka the "LEN" (length) of the array.
        So, this function's job is to play with the "LEN" to increase cache-references.
        
        Because the method to increase cache-references is the same as increasing cache-misses, this 
        method just calls that method.
        """
        self.__helperForCacheReferences(increment, extraPrints)

    def decreaseCacheReferences(self, increment, extraPrints=False):
        """
        Important: Call setInitialLENForCacheMisses() before calling this method!
        
        This method's job is to decrease perf's "cache-references" counters. This is done by adding
        a function that copies data from an array to another array that is far (1MB) away in memory.
        We ran tests and found that doing so will decrease the "cache-references" counter. The amount 
        of decrease can be controlled by how big those arrays are, aka the "LEN" (length) of the array.
        So, this function's job is to play with the "LEN" to decrease cache-references.
        
        Because the method to decrease cache-references is the same as increasing cache-misses, this 
        method just calls that method.
        """
        return self.__helperForCacheReferences(-1 * increment, extraPrints)

    def __helperForCacheReferences(self, increment, extraPrints=False):
        """
        Private method.
...
##############################################################################
    def increaseBranchInstRetiredAllBranches(self, initial_value, extraPrints=False):
        ...
        #     for (int y = 0; y < X; ++y) {{
        #         for (int z = 0; z < X; ++z) {{
        #             w += y * z; // No memory access to reduce side effect in the cache/memory domain
        #         }}
        #     }}
...
##############################################################################
    ...
    def impactBranchMissPredictRetired(self,initial_value,extraPrints=False):
        ...
        #         for (int z = 0; z < X; ++z) {{
        #             w += y * z; // No memory access to reduce side effect in the cache/memory domain
        #             if ((w % 3 == 0) || (w % 27 == 0) || (w % 8 == 0)) 
        #                 continue; // Increase Branch miss prediction rate
        #         }}
        #     }}
...
##############################################################################
    def decreaseTotalInstrCompleted(self, initial_value, extraPrints=False):
        # TODO UNTESTED, this is fuzzing PAPI_TOT_INS
        # Generate the fuzz string
        fuzz_string = f"""\
            clock_t local_temp_start = clock();
...
###############################################################################
# Author: Debopriya Roy Dipta
# Module 6: Flush-Reload (randomizing attack )
####################################################################################
